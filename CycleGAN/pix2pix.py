...
def discriminator(input_ch = 2, output_ch =1 , norm_type='batchnorm', target=True ):
  ...
  inp = tf.keras.layers.Input(shape=[None, None, input_ch ], name='input_image')
  x = inp

  if target:
    tar = tf.keras.layers.Input(shape=[None, None, output_ch ], name='target_image')
...
class Pix2pix(object):
  ...
  def __init__(self, epochs, enable_function):
    ...
    self.generator = unet_generator(output_channels=1, input_channels=2)
