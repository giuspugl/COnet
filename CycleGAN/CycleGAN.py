    ...
    def __init__(self, input_shape=(2, 128, 128), num_residual_blocks=6):
        ...
        for _ in range(2):
            model += [
                ...
                #--------------------to avoid checkerboard effects on predictions 
                # Step A: Scale up the image size (2x)
                #nn.Upsample(scale_factor=2, mode='nearest'), 
                
                # Step B: Refine features with standard Conv
                #nn.ReflectionPad2d(1),
                #nn.Conv2d(in_features, out_features, 3, stride=1, padding=0),
                #-----------------------------
                nn.InstanceNorm2d(out_features),
                nn.ReLU(inplace=True)
            ]
...
def get_memory_loader(input_path_A , input_path_B ,mode,  batch_size =4, shuffle=True):
    """
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=4, pin_memory=True # Speed up transfer to GPU
    )
...
def train(input_path_A=None, input_path_B=None, epochs=100,mode='unpaired', save_dir="./", 
    ...
    scaler = GradScaler('cuda') #GradScaler()
    # 2. Load Data
...
if __name__ == "__main__":
    ...
    import glob 
    import re
    #chks = (glob . glob (f"{workdir}/extending_CO/new_experiment/checkpoint_epoch_*" )) 
    ...
    train(f"{workdir}/data/feature_file_training.pt", f"{workdir}/data/target_file_training.pt", 
          epochs=500, mode= 'unpaired',
          batch_size=32, 
          save_dir = f"{workdir}/unpaired_training_norm" ,resume_path=latest_chkp ) 
